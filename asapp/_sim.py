...
def generate_H(N, K, alpha=1., eps=10.):
	H = sample_gamma(alpha, 1., size=(K, N))
	for k in range(K):
		size = H[k:k+1, int(k*N/K):int((k+1)*N/K)].shape
		H[k:k+1, int(k*N/K):int((k+1)*N/K)] =  sample_gamma(0.05, 1./1000., size=size)
		H[k:k+1, int(k*N/K):int((k+1)*N/K)] = sample_gamma(alpha + eps, 1./eps, size=size)
	return H

def generate_W(P, K, noise_prop=0., beta=1., eps=10.):
	...
	for k in range(K):
		...
		W[int(k*P_0/K):int((k+1)*P_0/K), k:k+1] = sample_gamma(0.05, 1./1000., size=size)	

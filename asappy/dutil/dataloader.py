...
class DataSet:
	...
	def _estimate_batch_mode(self,dataset_list,batch_size):
		...
		self.uns['batch_size'] = batch_size
		...
		if len(self.uns['dataset_list']) == 1:
			...
			if n_cells < self.uns['batch_size']:
				self.uns['run_full_data'] = True
				self.uns['dataset_batch_size'][self.uns['dataset_list'][0]] = n_cells

			else:
				self.uns['run_full_data'] = False
				self.uns['dataset_batch_size'][self.uns['dataset_list'][0]] = batch_size
		else:
			n_cells = 0 
			...
			if n_cells < self.uns['batch_size']:
				self.uns['run_full_data'] = True
				for ds in self.uns['dataset_list']:
					self.uns['dataset_batch_size'][ds] = f[ds]['shape'][()][0]
	...
	def initialize_data(self,dataset_list,batch_size):
		...
		if total_datasets == 1 and self.uns['run_full_data'] :
			...
			for ds in self.uns['dataset_list']:
				start_index = 0
				end_index = self.uns['dataset_batch_size'][ds]
	...
	def load_datainfo_batch(self,batch_index,start_index, end_index):
		...
		if len(self.uns['dataset_list']) == 1:
			...
			for ds in self.uns['dataset_list']:
				...
				end_index =  self.uns['dataset_batch_size'][ds] * batch_index
				start_index = 	end_index - self.uns['dataset_batch_size'][ds]			

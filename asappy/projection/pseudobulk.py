...
def generate_pseudobulk_batch(asap_object,batch_i,start_index,end_index,rp_mat,result_queue,lock,sema):
    ...
    if batch_i <= asap_object.number_batches:
        ...
        get_pseudobulk(local_mtx.T, 
            rp_mat, 
            asap_object.downsample_pseudobulk,asap_object.downsample_size,
            str(batch_i) +'_' +str(start_index)+'_'+str(end_index),
            result_queue
            )
        sema.release()			
    else:
        logging.info('Pseudo-bulk NOT generated for '+str(batch_i) +'_' +str(start_index)+'_'+str(end_index)+ ' '+str(batch_i) + ' > ' +str(asap_object.number_batches))

def filter_pseudobulk(asap_object,pseudobulk_result,min_size=5):

    asap_object.pseudobulk = {}
    ...
    if len(pseudobulk_result) == 1 and asap_object.adata.uns['run_full_data']:
        ...
        asap_object.pseudobulk['pb_data'] = pseudobulk_result['full']['pb_data'][:,keep_indices]
        ...
        asap_object.pseudobulk['pb_map'] = {batch_index:pseudobulk_indices}

    else:
        asap_object.pseudobulk['pb_map'] = {}
        for indx,result_batch in enumerate(pseudobulk_result):
            ...
            if indx == 0:
                asap_object.pseudobulk['pb_data'] = pb
            else:
                asap_object.pseudobulk['pb_data'] = np.hstack((asap_object.pseudobulk['pb_data'],pb))
            
            asap_object.pseudobulk['pb_map'][[k for k in result_batch.keys()][0]] = pseudobulk_map
...
def generate_pseudobulk(asap_object,tree_depth,downsample_pseudobulk=True,downsample_size=100,maxthreads=16,pseudobulk_filter_size=5):
    ...
    if total_cells<batch_size:
        ...
        pseudobulk_result = get_pseudobulk(asap_object.adata.X.T, rp_mat,asap_object.downsample_pseudobulk,asap_object.downsample_size,'full')

...
result_file1 = wdir+sample+'_nmf_eval.csv'
result_file2 = wdir+sample+'_result.csv'
print(sample)

model_list = ['asap']

sample_file=wdir+sample+'_memory_profiler_output.txt'
...
def get_mem(si,ei):
    memsum = df2.loc[(df2[0] >= si) & (df2[0] <= ei), 3].sum() 
    startmem = df2.iloc[0,3]
    memu = memsum - startmem
    return memu

asap_mem = get_mem(49,85)

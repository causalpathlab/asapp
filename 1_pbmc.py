import sys
sys.path.append('../')
from asappy.util.io import read_config
from collections import namedtuple
from pathlib import Path
import pandas as pd
import numpy as np
from asappy.data.dataloader import DataSet
...
df_corr.index = [ x.split('@')[0] for x in model['predict_barcodes']]
...
# assign celltype
df = pd.read_csv(dl.outpath+'_scanpy_label.csv.gz')
df['cell'] = [ x.split('@')[0] for x in df['cell'].values]
df_umap = pd.merge(df_umap,df,on='cell',how='left')
df_umap = df_umap[~df_umap.leiden.isna()]

########### pre bc
df_corr = df_corr.loc[df_umap['cell'].values,:]
df_umap[['umap_1','umap_2']] = analysis.get2dprojection(df_corr.to_numpy())
df_umap.drop(columns=['umap1','umap2'],inplace=True)
...
from asappy.util import batch_correction as bc 
